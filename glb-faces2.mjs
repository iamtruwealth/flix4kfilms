...
loader.parse(data.buffer, '', (gltf) => {
  ...
  const GRID = 96; const X0=-2.6, X1=2.6, Y0=-0.1, Y1=3.85
  ...
  scene.traverse((obj) => {
    ...
    for (let t=0;t<triCount;t++){
      ...
      const gx0=Math.max(0,Math.floor((Math.min(a.x,b.x,c.x)-X0)/(X1-X0)*GRID)), gx1=Math.min(GRID-1,Math.floor((Math.max(a.x,b.x,c.x)-X0)/(X1-X0)*GRID))
      const gy0=Math.max(0,Math.floor((Math.min(a.y,b.y,c.y)-Y0)/(Y1-Y0)*GRID)), gy1=Math.min(GRID-1,Math.floor((Math.max(a.y,b.y,c.y)-Y0)/(Y1-Y0)*GRID))
      for (let gy=gy0;gy<=gy1;gy++) for (let gx=gx0;gx<=gx1;gx++){ const k=gy*GRID+gx; if (zM<cell[k]) cell[k]=zM }
    }
  })
  const q = 0.03
  ...
  for (let gy=0;gy<GRID;gy++) for (let gx=0;gx<GRID;gx++){
    ...
    const z=qz(cell[k]); const stack=[k]; comp[k]=compId
    const info={n:0,minX:Infinity,maxX:-Infinity,minY:Infinity,maxY:-Infinity,z}
    while(stack.length){ const cur=stack.pop(); const cx=cur%GRID, cy=(cur/GRID)|0
      info.n++
      info.minX=Math.min(info.minX,cx); info.maxX=Math.max(info.maxX,cx); info.minY=Math.min(info.minY,cy); info.maxY=Math.max(info.maxY,cy)
    ...
    infos.set(compId, info); compId++
  }
  const rows=[]
  for (const [id,i] of infos){
    if (i.n<40) continue
    const w=(i.maxX-i.minX+1)/GRID*(X1-X0), h=(i.maxY-i.minY+1)/GRID*(Y1-Y0)
    const cx=(i.minX+i.maxX+1)/2/GRID*(X1-X0)+X0
    const cy=Y1-(i.minY+i.maxY+1)/2/GRID*(Y1-Y0)
    rows.push({n:i.n,w,h,cx,cy,z:i.z*q,aspect:(w/h).toFixed(2)})
  }
  rows.sort((p,qq)=>qq.n-p.n)
  for (const r of rows.slice(0,12)) console.log(`cells ${String(r.n).padStart(4)}  z ${r.z.toFixed(2)}  w ${r.w.toFixed(2)} h ${r.h.toFixed(2)}  aspect ${r.aspect}  center ${r.cx.toFixed(2)},${r.cy.toFixed(2)}`)
